...
/**
 * @author ：Yunchenyan
 * @date ：Created in 2024/11/25 22:28
 * description：
 */
public class WaterMarkCustomerDemo {
                ...
                .assignTimestampsAndWatermarks(
                        ...
                        WatermarkStrategy.<WaterSensor>forGenerator(new WatermarkGeneratorSupplier<WaterSensor>() {
                            ...
                            public WatermarkGenerator<WaterSensor> createWatermarkGenerator(Context context) {
                                return new WaterMarkCustomerGenerator<>(3000);
                            }
                        })
                                // 指定时间戳分配器 从数据中提取
...
class WaterMarkCustomerGenerator<T> implements WatermarkGenerator<T> {
    ...
    public WaterMarkCustomerGenerator(long delayTs) {
        this.delayTs = delayTs;
        this.maxTs = Long.MIN_VALUE + delayTs + 1L;
    }

    /**
     * 每条数据来 都会调用一次 用于提取事件时间戳，并且保存下载
     *
     * @param event
    ...
    public void onEvent(T event, long eventTimestamp, WatermarkOutput output) {
        ...
        System.out.println("调用了onEvent, maxTs=" + maxTs);
    }
    ...
    public void onPeriodicEmit(WatermarkOutput output) {
        ...
        System.out.println("调用了onPeriodicEmit, 生成的watermark=" + (maxTs - delayTs - 1));
    }
}
